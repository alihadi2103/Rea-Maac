...
class PGTrainer(object):
    ...
    def run(self, stat, episode):
        ...
        ep_rewards=self.behaviour_net.train_process(stat, self)
...
class DPGTrainer(object):
    def __init__(self, args, model, env, logger):
        ...
        # Setup Replay Buffer
        if self.args.replay:
            buffer_cls = EpisodeReplayBuffer if self.episodic else TransReplayBuffer
            self.replay_buffer = buffer_cls(int(self.args.replay_buffer_size))

        self.env = env

        self.policy_optimizer = optim.RMSprop(
            self.behaviour_net.policy_dicts.parameters(), lr=args.policy_lrate, alpha=0.99, eps=1e-5
        )
        self.value_optimizer = optim.RMSprop(
            self.behaviour_net.value_dicts.parameters(), lr=args.value_lrate, alpha=0.99, eps=1e-5
        )
    ...
    def get_loss(self, batch):
        policy_loss, value_loss= self.behaviour_net.get_loss(batch)
        return policy_loss, value_loss, 

    def policy_compute_grad(self, loss, retain_graph):
        """
        Handles entropy regularization and gradient computation for continuous or discrete action spaces.
        """
    ...
    def _sample_and_process(self, process_fn, stat):
        batch = self.replay_buffer.get_batch(self.args.batch_size)
        batch = self.behaviour_net.Transition(*zip(*batch))
        process_fn(stat, batch)

    def policy_replay_process(self, stat):
        self._sample_and_process(self.policy_transition_process, stat)

    def value_replay_process(self, stat):
        self._sample_and_process(self.value_transition_process, stat)

    def mixer_replay_process(self, stat):
        self._sample_and_process(self.mixer_transition_process, stat)

    def policy_transition_process(self, stat, trans):
        policy_loss, _,  = self.get_loss(trans)
        self.policy_optimizer.zero_grad()

        self.policy_compute_grad( policy_loss, False)
        

        self.grad_clip(self.policy_optimizer.param_groups[0]['params'])
        self.policy_optimizer.step()

        stat['mean_train_policy_grad_norm'] = get_grad_norm(self.args, self.policy_optimizer.param_groups[0]['params']).item()
        stat['mean_train_policy_loss'] = policy_loss.clone().mean().item()

    def value_transition_process(self, stat, trans):
        _, value_loss = self.get_loss(trans)
        self.value_optimizer.zero_grad()
        self.value_compute_grad(value_loss, False)
        self.grad_clip(self.value_optimizer.param_groups[0]['params'])
        self.value_optimizer.step()

        stat['mean_train_value_grad_norm'] = get_grad_norm(self.args, self.value_optimizer.param_groups[0]['params']).item()
        stat['mean_train_value_loss'] = value_loss.clone().mean().item()
    ...
    def mixer_transition_process(self, stat, trans):
        ...
        self.mixer_optimizer.zero_grad()
        self.value_compute_grad(value_loss, False)
        self.grad_clip(self.mixer_optimizer.param_groups[0]['params'])
        self.mixer_optimizer.step()

        stat['mean_train_mixer_grad_norm'] = get_grad_norm(self.args, self.mixer_optimizer.param_groups[0]['params']).item()
        stat['mean_train_mixer_loss'] = value_loss.clone().mean().item()

    def run(self, stat, episode):
        stat=self.behaviour_net.train_process(stat, self)
